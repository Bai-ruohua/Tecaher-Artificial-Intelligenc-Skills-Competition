...
def init_db():
    ...
    # 改造 courses：补 textbook 列（v4.7）
    try:
        c.execute("ALTER TABLE courses ADD COLUMN textbook TEXT DEFAULT ''")
    except sqlite3.OperationalError:
        pass
    ...
    # 知识切片标题列（v4.4）
    try:
        c.execute("ALTER TABLE course_documents ADD COLUMN title TEXT DEFAULT ''")
    except sqlite3.OperationalError:
        pass
...
def create_course(teacher_id, course_name, semester="", description="", textbook=""):
    conn = get_db()
    c = conn.execute(
        "INSERT INTO courses (teacher_id, course_name, semester, description, textbook) VALUES (?, ?, ?, ?, ?)",
        (teacher_id, course_name, semester, description, textbook)
    )
    conn.commit()
    cid = c.lastrowid
    conn.close()
...
def save_course_document(course_id, chunk_text, source_file="", chapter_id="", title=""):
    conn = get_db()
    # 检查是否有 title 列（兼容旧数据库）
    cols = [r[1] for r in conn.execute("PRAGMA table_info(course_documents)").fetchall()]
    if "title" in cols:
        c = conn.execute(
            """INSERT INTO course_documents (course_id, chunk_text, source_file, chapter_id, title)
               VALUES (?, ?, ?, ?, ?)""",
            (course_id, chunk_text, source_file, chapter_id, title)
        )
    else:
        c = conn.execute(
...
def list_course_documents(course_id, limit=500):
    """轻量列表：仅返回文档元数据（含标题预览），用于知识库页面快速展示"""
    conn = get_db()
    cols = [r[1] for r in conn.execute("PRAGMA table_info(course_documents)").fetchall()]
    select_cols = "id, course_id, source_file, chapter_id, created_at"
    if "title" in cols:
        select_cols += ", title"
    rows = conn.execute(
        f"SELECT {select_cols} FROM course_documents WHERE course_id=? ORDER BY id ASC LIMIT ?",
        (course_id, limit)
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def delete_course(course_id, teacher_id=None):
    """
    级联删除课程及所有关联数据。
    如果指定 teacher_id，则先校验课程归属再删除。
    返回 (success, message)
    """
    conn = get_db()

    # 1. 校验课程存在
    course = conn.execute("SELECT id, teacher_id, course_name FROM courses WHERE id=?", (course_id,)).fetchone()
    if not course:
        conn.close()
        return (False, "课程不存在")
    ...
    if teacher_id and course["teacher_id"] != teacher_id:
        conn.close()
        return (False, "无权删除此课程")

    course_name = course["course_name"]

    # 3. 级联删除所有关联表数据（按 course_id）
    cascade_tables = [
        "course_chapters",      # 章节体系
        "course_documents",     # 知识库切片
        "enrollments",          # 选课记录
        "grades",               # 成绩
        "student_attempts",     # 答题记录
        "wrong_books",          # 错题本
        "mastery_timeline",     # 掌握度时间线
        "lesson_plans",         # 教案
        "questions",            # 题库
        "prompt_cache",         # AI 缓存
        "ability_scores",       # 能力评分
    ...
    for tbl in cascade_tables:
        try:
            c = conn.execute(f"DELETE FROM {tbl} WHERE course_id=?", (course_id,))
            deleted_counts[tbl] = c.rowcount
    ...
    return (True, f"课程「{course_name}」及所有关联数据已删除", deleted_counts)
