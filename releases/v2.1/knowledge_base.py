...
CHAPTERS = [
    {
        ...
        "practice_questions": [
            {"q": "Python的创始人是谁？", "a": "Guido van Rossum（吉多·范罗苏姆）"},
            {"q": "pip install 命令的作用是什么？", "a": "安装Python第三方包/库"},
        ],
        "code_examples": [
            'print("Hello, World!")',
    ...
    },
    {
        ...
        "objectives": [
            "掌握Python变量的命名规则和赋值方式",
            "熟练使用int、float、str、bool四种基本数据类型",
            "掌握类型转换和格式化输出"
        ],
        "key_points": ["变量命名规范（字母/下划线开头）", "type()查看类型", "int()/float()/str()类型转换", "f-string格式化"],
        "difficulties": ["float精度问题", "str和int/float的转换陷阱"],
        "practice_questions": [
            {"q": "name = input('请输入姓名')的返回值是什么类型？", "a": "字符串（str）类型"},
            {"q": "如何将一个字符串'3.14'转为浮点数？", "a": "float('3.14')"},
        ],
        "code_examples": [
            'name = input("请输入姓名：")\nprint(f"你好，{name}！")',
    ...
    },
    {
        ...
        "practice_questions": [
            {"q": "7 // 2 的结果是多少？", "a": "3（整除，向下取整）"},
            {"q": "3 > 2 and 1/0 会报错吗？", "a": "会报错，因为and两边都会评估（Python中无短路优化区别）"},
        ],
        "code_examples": [
    ...
    },
    {
        ...
        "practice_questions": [
            {"q": "elif和else if有什么区别？", "a": "Python中没有else if关键字，统一用elif"},
        ],
        "code_examples": [
    ...
    },
    {
        ...
        "practice_questions": [
            {"q": "range(1, 10, 2)产生的序列是什么？", "a": "[1, 3, 5, 7, 9]"},
            {"q": "break和continue的区别是什么？", "a": "break直接退出整个循环，continue跳过本次迭代继续下一次"},
        ],
        "code_examples": [
    ...
    },
    {
        ...
        "practice_questions": [
            {"q": "s = 'abcde'; s[1:4]的结果是什么？", "a": "'bcd'"},
            {"q": "如何判断一个字符串是否是纯数字？", "a": "s.isdigit()"},
        ],
        "code_examples": [
    ...
    },
    {
        ...
        "practice_questions": [
            {"q": "list和tuple的主要区别是什么？", "a": "list可变（可增删改），tuple不可变（创建后不能修改）"},
        ],
        "code_examples": [
    ...
    },
    {
        ...
        "practice_questions": [
            {"q": "d = {}; d[[1,2,3]] = 'value' 会报什么错误？", "a": "TypeError，因为list是可变类型不能作为字典的键"},
        ],
        "code_examples": [
    ...
    },
    {
        ...
        "practice_questions": [
            {"q": "lambda x, y: x + y 的含义是什么？", "a": "一个匿名函数，接收两个参数x和y，返回它们的和"},
        ],
        "code_examples": [
            'def fibonacci(n):\n    a, b = 0, 1\n    for _ in range(n):\n        print(a, end=" ")\n        a, b = b, a + b\nfibonacci(10)'
        ]
    },
    {
        ...
        "practice_questions": [
            {"q": "with open('test.txt', 'r', encoding='utf-8') as f 的作用是什么？", "a": "以只读模式打开文件，指定UTF-8编码，并在代码块结束后自动关闭文件"},
        ],
        "code_examples": [
    ...
    },
    {
        ...
        "practice_questions": [
            {"q": "class Dog(Animal)中Dog的父类是什么？", "a": "Animal"},
            {"q": "self参数代表什么？", "a": "self代表类的实例对象本身"},
        ],
        "code_examples": [
    ...
    },
    {
        ...
        "practice_questions": [
            {"q": "综合项目开发的基本流程是什么？", "a": "需求分析→概要设计→详细设计→编码实现→测试→部署维护"},
        ],
        "code_examples": [
...
def get_all_chapters():
    """获取所有章节信息"""
    return [{k: v for k, v in ch.items() if k != "code_examples"} for ch in CHAPTERS]
...
def get_chapter_text(chapter_id):
    ...
    parts = [
        ...
        f"教学目标：{'；'.join(ch['objectives'])}",
        f"重点内容：{'；'.join(ch['key_points'])}",
        f"难点：{'；'.join(ch['difficulties'])}",
    ]
    return "\n".join(parts)
...
def search_by_keyword(keyword):
    ...
    for ch in CHAPTERS:
        text = " ".join(ch["keywords"]).lower() + " " + ch["title"].lower()
