...
def generate_questions(chapter_id, q_type="choice", count=5, difficulty="B"):
    """生成题目"""
    ...
    return deepseek.chat("你是Python课程命题专家，擅长根据知识点出高质量的题目。", prompt, max_tokens=4096)


def generate_mixed_paper(chapter_id, config=None):
    """生成完整试卷"""
    ch = get_chapter(chapter_id)
    if not ch:
        return "章节不存在"
