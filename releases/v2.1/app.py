# -*- coding: utf-8 -*-
"""
AI智能教学平台 - Flask主应用 (V2.1 师生分离+认证+数据同步)
宜宾工业职业技术学院 · 数字经济学院
...
from knowledge_base import get_all_chapters, get_chapter, search_by_keyword
...
from auth import (
    ...
    init_default_teacher, hash_password,
...
DEFAULT_CHAPTERS = get_all_chapters()
...
def teacher_dashboard_page():
    ...
    courses = get_courses_by_teacher(user["user_id"])
    return render_template("teacher/dashboard.html", teacher=teacher, courses=courses, chapters=DEFAULT_CHAPTERS, config=Config)
...
def student_dashboard_page():
    ...
    return render_template("student/dashboard.html", student=student, courses=courses, chapters=DEFAULT_CHAPTERS, config=Config)
...
def lesson_prep_page():
    return render_template("teacher/lesson_prep.html", chapters=DEFAULT_CHAPTERS, config=Config)

@app.route("/teacher/exam")
@teacher_required
def exam_page():
    return render_template("teacher/exam.html", chapters=DEFAULT_CHAPTERS, config=Config)

@app.route("/teacher/grading")
@teacher_required
def grading_page():
    return render_template("teacher/grading.html", chapters=DEFAULT_CHAPTERS, config=Config)

@app.route("/teacher/analytics")
@teacher_required
def analytics_page():
    return render_template("teacher/analytics.html", chapters=DEFAULT_CHAPTERS, config=Config)
...
def student_management_page():
    ...
    teacher = get_teacher_by_id(user["user_id"])
    courses = get_courses_by_teacher(user["user_id"])
    return render_template("teacher/students.html", teacher=teacher, courses=courses, chapters=DEFAULT_CHAPTERS, config=Config)
...
@student_required
def student_qa_page():
    return render_template("student/agent_qa.html", chapters=DEFAULT_CHAPTERS, config=Config)

@app.route("/student/quiz")
@student_required
def student_quiz_page():
    return render_template("student/agent_quiz.html", chapters=DEFAULT_CHAPTERS, config=Config)

@app.route("/student/digital")
@student_required
def student_digital_page():
    return render_template("student/agent_digital.html", chapters=DEFAULT_CHAPTERS, config=Config)

@app.route("/student/graph")
@student_required
def student_graph_page():
    return render_template("student/agent_graph.html", chapters=DEFAULT_CHAPTERS, config=Config)
...
def api_student_dashboard():
    user = get_current_user()
    data = get_student_dashboard(user["user_id"])
    return jsonify({"success": True, "data": data})
...
def api_student_radar():
    user = get_current_user()
    radar = compute_radar_data(user["user_id"])
    return jsonify({"success": True, "radar": radar})
...
def api_student_gantt():
    user = get_current_user()
    gantt = compute_gantt_data(user["user_id"])
    return jsonify({"success": True, "gantt": gantt})
...
def api_lesson_plan():
    ...
    result = generate_lesson_plan(
        data.get("chapter_id"), data.get("extra_requirements", "")
    )
    ch = get_chapter(data.get("chapter_id"))
    if ch:
...
def api_courseware():
    data = request.json
    result = generate_courseware(data.get("chapter_id"), data.get("lesson_plan_text", ""))
    return jsonify({"success": True, "content": result})
...
def api_ideology():
    data = request.json
    result = generate_ideology(data.get("chapter_id"), data.get("topic", ""))
    return jsonify({"success": True, "content": result})
...
def api_practice():
    data = request.json
    result = generate_practice(data.get("chapter_id"))
    return jsonify({"success": True, "content": result})
...
def api_questions():
    ...
    result = generate_questions(
        ...
        data.get("count", 5), data.get("difficulty", "B")
    )
    return jsonify({"success": True, "content": result})
...
def api_exam_paper():
    data = request.json
    result = generate_mixed_paper(data.get("chapter_id"), data.get("config"))
    return jsonify({"success": True, "content": result})
...
def api_grade():
    data = request.json
    result = grade_code(
        data.get("code", ""), data.get("task_description", ""), data.get("chapter_id")
    )
    return jsonify({"success": True, "content": result})
...
def api_diagnose():
    data = request.json
    result = diagnose_bug(
        data.get("code", ""), data.get("error_message", ""), data.get("chapter_id")
    )
    return jsonify({"success": True, "content": result})
...
def api_student_qa():
    ...
    try:
        result = student_qa(student_id, question, history)
...
def api_chapter_qa_guide(chapter_id):
    questions = get_chapter_qa_guide(chapter_id)
...
def api_quiz_generate():
    ...
    try:
        result = generate_quiz_questions(chapter_id, count, difficulty)
        return jsonify({"success": True, "data": result})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_wrong_book():
    ...
    summary = get_wrong_book_summary(wrong_items)
...
def api_digital_explain():
    ...
    try:
        from modules.student_agents import student_qa
        qa_result = student_qa(student_id, topic or f"请讲解{chapter_id}章节的内容")
...
def api_digital_pre_generate():
    data = request.json
    chapter_id = data.get("chapter_id")
    result = digital_human.pre_generate_chapters(chapter_id)
    return jsonify({"success": True, "data": result})
...
def api_graph_detail(chapter_id):
    detail = get_chapter_detail_for_graph(chapter_id)
    if not detail:
...
def api_chapters():
    return jsonify({"success": True, "chapters": DEFAULT_CHAPTERS})
