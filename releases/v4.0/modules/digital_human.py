...
class DigitalHumanEngine:
    ...
    def pre_generate_chapters(self, chapter_id=None, course_id=None):
        """
        预生成章节讲解视频（批量缓存）
        为高频知识点提前生成数字人讲解视频（按课程章节）
        """
        from knowledge_base import get_course_chapter, get_course_chapters
        ...
        if chapter_id:
            ...
            tasks = [c["chapter_id"] for c in get_course_chapters(course_id)] or [f"ch{i:02d}" for i in range(1, 13)]

        for cid in tasks:
            ch = get_course_chapter(course_id, cid)
            if not ch:
...
本章节的重点内容包括：{'；'.join(kps[:5])}。

学习本章之前，你需要掌握：{'；'.join(objs[:2])}。

{'；'.join(diffs)} 是本章的难点，建议多花时间练习。
