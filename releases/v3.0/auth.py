...
def seed_default_courses():
    ...
    from database import get_courses_by_teacher, create_course, seed_course_chapters_from_seed
    from knowledge_base import SEED_PYTHON_CHAPTERS_ENRICHED
    teachers = get_all_teachers()
    if not teachers:
        return
    admin = teachers[0]
    ...
    seed_course_chapters_from_seed(cid, SEED_PYTHON_CHAPTERS_ENRICHED)
    return cid


# ===== 课程管理逻辑 =====

def handle_create_course(teacher_id, course_name, semester="", description=""):
    cid = create_course(teacher_id, course_name, semester, description)
    return cid
